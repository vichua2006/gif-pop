...
export function updateGif(id, updates) {
    const gif = data.gifs.find(g => g.id === id);
    if (gif) {
        if (updates.name !== undefined)
            gif.name = updates.name;
        if (updates.is_favorite !== undefined)
            gif.is_favorite = updates.is_favorite;
        saveDatabase();
    }
}
...
export function insertTag(name) {
    const tag = { id: data.nextTagId++, name };
    data.tags.push(tag);
    saveDatabase();
    return tag;
}
...
export function addTagToGif(gifId, tagId) {
...
export function removeTagFromGif(gifId, tagId) {
    data.gifTags = data.gifTags.filter(gt => !(gt.gif_id === gifId && gt.tag_id === tagId));
    saveDatabase();
}
export function getTagsForGif(gifId) {
...
export function getGifIdsByTag(tagId) {
